...
class Budget(models.Model):
    ...
    total_income = models.DecimalField(
        _('Toplam Gelir'), null=True, blank=True, decimal_places=5, max_digits=10)
    total_outcome = models.DecimalField(
        _('Toplam Gider'), null=True, blank=True, decimal_places=5, max_digits=10)
    created_at = models.DateTimeField(
        _('Kayıt Tarihi'), auto_now_add=True, editable=False)
    updated_at = models.DateTimeField(
        _('Güncellenme Tarihi'), auto_now=True, editable=False)
...
def set_income_total(sender, **kwargs):
    ...
    if instance.id is None:
        total_income = Budget.objects.filter(user=instance.user).exclude(
            product_order__isnull=True).first()
        ...
        if total_income is not None:
            ...
            if instance.product_order:
                instance.total_income = Decimal(
                    instance.product_order.unit_price * instance.product_order.quantitiy)
...
def set_outcome_total(sender, **kwargs):
    instance = kwargs["instance"]
    if instance.id is None:
        total_outcome = Budget.objects.filter(
            user=instance.user).exclude(raw_order__isnull=True).first()
